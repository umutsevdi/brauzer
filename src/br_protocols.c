...
BR_PRT_STATUS
br_http_response_new(BrHttpResponse* h_resp, char* resp, size_t resp_s)
{
...
static void
_http_response_destroy_kv(gpointer key, gpointer value, gpointer user_data)
{
    free(key);
    free(value);
}
...
static BR_PRT_STATUS
_parse_gem_headers(char* resp, size_t resp_s, const char* endptr,
                   BrGemResponse* r);

BR_PRT_STATUS
br_gemini_response_new(BrGemResponse* gem_r, char* resp, size_t resp_s)
{
    char* l_begin = resp;
    char* l_end = resp;
    if ((l_end = strstr(l_begin, "\r\n")) != NULL) {
